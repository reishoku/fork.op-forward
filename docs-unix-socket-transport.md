...
```bash
ssh -fN -R ${XDG_RUNTIME_DIR:-$HOME/.cache}/op-forward.sock:$HOME/Library/Caches/op-forward/op-forward.sock vm
```

Set `OP_FORWARD_SOCKET_PATH=${XDG_RUNTIME_DIR:-$HOME/.cache}/op-forward.sock` on the remote client side.
